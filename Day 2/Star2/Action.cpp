...
Action::Action(char action_, int condition) {

	if (condition == tie) action = Play(action_ - 'A' + 1);
	if (condition == win ) {

		if (action_ != 'C') action = Play(action_ - 'A' + 2);
		else action = rock;

	}
	if (condition == lose) {

		if (action_ != 'A') action = Play(action_ - 'A');
...
Play Action::get_play() { return action; }
